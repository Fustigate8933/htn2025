export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Start the batch generation in the background
    ...
    console.error('Async batch video generation error:', error)
    
    // If it's a timeout, return a partial response indicating processing is ongoing
    if (error.message?.includes('timeout') || error.message?.includes('Headers Timeout')) {
      return {
        success: true,
        message: "Batch generation started successfully, processing in background",
        status: "processing",
        total_videos: JSON.parse(texts).length,
        video_urls: [] // Will be populated when complete
      }
    }
    
    throw createError({ 
      statusCode: 500, 
      statusMessage: "Failed to start batch video generation" 
    })
  }
})
